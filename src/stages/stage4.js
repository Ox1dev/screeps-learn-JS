...
const stage4 = {
    run: function (spawn, numOfCreeps) {
        if (numOfCreeps.harvester < 1 && !spawn.spawning && spawn.room.energyAvailable >= 1300) {
            for (let i = 0; i <= 2; i++) {
        ...
        } else if (numOfCreeps.hauler < 2 && !spawn.spawning && spawn.room.energyAvailable >= 1300) {
            for (let i = 0; i <= 2; i++) {
                ...
                spawn.spawnCreep([WORK,WORK,WORK,WORK,WORK,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,MOVE,MOVE,
                        MOVE,MOVE,MOVE,MOVE,MOVE], newHauler,
                    {memory: {role: 'hauler'}});
