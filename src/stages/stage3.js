...
const stage3 = {
    run: function (spawn, numOfCreeps) {
        if (numOfCreeps.harvester < 1 && !spawn.spawning && spawn.room.energyAvailable >= 800) {
            for (let i = 0; i <= 2; i++) {
        ...
        } else if (numOfCreeps.hauler < 2 && !spawn.spawning && spawn.room.energyAvailable >= 800) {
            for (let i = 0; i <= 2; i++) {
                let newHauler = 'Hauler' + Game.time;
                spawn.spawnCreep([WORK,WORK,WORK,CARRY,CARRY,CARRY,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE], newHauler,
                    {memory: {role: 'hauler'}});
